...
function controller!(
    ...
    @infinite_parameter(
        model,
        t ∈ [timeframe[1], timeframe[2]],
        num_supports = length(extra_ts) + 2,
    ...
    @constraint(
        ...
        S_constr,
        ∂(S, t) ==
        -(1 - υ) * parameters[1] * parameters[2] * S * I + parameters[4] * R -
        parameters[7] * S
    )
    @constraint(
        model,
        E_constr,
        ∂(E, t) == (1 - υ) * parameters[1] * parameters[2] * S * I - parameters[3] * E
    )
    @constraint(
        model,
        I_constr,
        ∂(I, t) == parameters[3] * E - parameters[2] * I - parameters[5] * I
    )
    @constraint(
        model,
        R_constr,
        ∂(R, t) ==
        (1 - parameters[5]) * parameters[2] * I - parameters[4] * R + parameters[7] * S
    )
