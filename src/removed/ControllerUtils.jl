...
function ude_prediction(
    ...
    function affect_voc!(integrator)
        ...
        integrator.p_true[2] =
            abs(rand(rng, Normal(integrator.p_true[2], integrator.p_true[2] / 10)))
        integrator.p_true[3] =
            abs(rand(rng, Normal(integrator.p_true[3], integrator.p_true[3] / 10)))
        integrator.p_true[4] =
            abs(rand(rng, Normal(integrator.p_true[4], integrator.p_true[4] / 10)))
        integrator.p_true[5] =
            abs(rand(rng, Normal(integrator.p_true[5], integrator.p_true[5] / 10)))
    end
    ...
    prob_nn =
        ODEProblem(nn_dynamics!, X[:, 1], (tspan[1], tspan[end]), p, callback = voc_cb)
...
function symbolic_regression(
    ...
    estimation_prob =
        ODEProblem(dynamics!, u0, tspan, get_parameter_values(nn_eqs), callback = voc_cb)
    estimate = solve(
        estimation_prob,
